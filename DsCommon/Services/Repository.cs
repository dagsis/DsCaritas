...
namespace DsCommon.Services
{
    public class Repository<T> : IRepository<T> where T : class
    {
        ...
        public async Task<IReadOnlyList<T>> GetAsync(Expression<Func<T, bool>>? predicate, Func<IQueryable<T>, IOrderedQueryable<T>>? orderBy = null, List<Func<IQueryable<T>, IIncludableQueryable<T, object>>> Includes = null, bool disableTracking = true)
        {
            IQueryable<T> query = _context.Set<T>();

            if (disableTracking) query = query.AsNoTracking();

            if (Includes != null) query = Includes.Aggregate(query, (current, include) => include(current));
